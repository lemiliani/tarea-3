program shield_1d
    ...
    ! This version incorporates Geometrical splitting and Exponential transformation
    !
    ! Author: Luis Emiliani, Andrés Aracena
    ! ruta: gfortran combinados.f90 mod_rng.f90 mod_mfp.f90 mod_scatt.f90
    ! run: ./a.out
...
implicit none
    ...
    real(kind=real64), dimension(nreg) :: xthick = 1.0      ! thickness of each region (cm)
    ...
    real(kind=real64), parameter :: c = 1.6         ! value given for the exponential transformation
    real(kind=real64), parameter :: rr_cut = 0.001  ! weight cut-off
    ...
    real(kind=real64) :: gs_n                        ! integer part of gs_r
    real(kind=real64), dimension(0:nreg) :: gs_i     ! importance of each region
    integer(kind=int32) ::  n = 5                    ! number used to divide the particles
    real(kind=real64) :: wact                        ! weight actual of the particle
            ...
            particle_loop: do
                ...
                ptrans_loop: do
                    ...
                    if(ir(ip) == 0 .or. ir(ip) == nreg+1) then
                        ...
                        pstep = mfp(sigma_t(ir(ip)),c,u(ip))
                        !Implementation of the exponential transformation
                        wt(ip) = wt(ip)*sigma_t(ir(ip))*exp(-c*u(ip)*pstep)/(sigma_t(ir(ip))-c*u(ip))
                    endif
                        ...
                        ! Implementation of geometrical splitting VRT.
                            ...
                            rnno = rng_set()
                                ...
                                if(rnno .le. 1.0 - (gs_r - gs_n)) then
                                    ...
                                    wact = wt(ip)
                                    if (ip + n - 1 .gt. nstack) then
                                        exit
                                    else
                                        do i = ip, ip + n - 1
                                            ! Update statistical weight and other 
                                            ! variables of the new particles
                                            wt(i) = wact/n
                                            x(i) = x(ip)
                                            u(i) = u(ip)
                                            ir(i) = ir(ip)
                                            ! Implement weight-cutoff using russian roulette.
                                            if(wt(i) .le. rr_cut) then
                                                rnno = rng_set()
                                                if(rnno .gt. 1.0/rr_d) then
                                                ! particle does not survive, finish history
                                                    exit
                                                else
                                                    ! particle survives, adjusts weight 
                                                    ! accordingly and continue transport.
                                                    wt(i) = rr_d*wt(i)                        
                                                endif 
                                            endif
                                        enddo
    ...
    write(*,'(A,F15.5)') 'Figure of merit (FOM) of Transmission: ', fom
