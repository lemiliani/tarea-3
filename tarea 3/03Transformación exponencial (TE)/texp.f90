program shield_1d
    ...
    ! Multi-region version, This version incorporates Exponential transformation
    !
    ! Authors: Luis Emiliani, Andrés Aracena
    ! ruta: gfortran texp.f90 mod_rng.f90 mod_mfp.f90 mod_scatt.f90
    ! run: ./a.out
            ...
            particle_loop: do
                ...
                ptrans_loop: do
                    ...
                    if(ir == 0 .or. ir == nreg+1) then
                        ...
                        pstep = mfp(sigma_t(ir), c, u)
                        !Implementation of the exponential transformation
                        wt = wt*sigma_t(ir)*exp(-c*u*pstep)/(sigma_t(ir)-c*u)
                    endif
